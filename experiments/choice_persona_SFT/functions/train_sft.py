...
from .evaluator import PersonaLogitEvalCallback
import logging
...
def train_sft(model_config, dataset_config, lora_config, training_config, 
    ...
    trainer = SFTTrainer(
        ...
        callbacks=[PersonaLogitEvalCallback(eval_dataset, tokenizer, batch_size=training_config.get("per_device_eval_batch_size"))],
    )
    try:
        trainer.train()
